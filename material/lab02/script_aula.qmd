...
dados <- "../lab01/Transferencias---Dados-Consolidados.xlsx" |> 
  readxl::read_excel(1, "C38:NJ38", col_names = FALSE, 
                     .name_repair = "minimal") |> 
  janitor::clean_names() |> 
  pivot_longer(everything()) |> 
  mutate(
    date = seq(as.Date("1991-01-01"), as.Date("2021-12-31"), "1 month"),
    value = value / 1e9
  ) |> 
  select(-name) |> 
  fill(value) |> 
...
fit <- tdata |> 
  model(
    arima_manual = ARIMA(value ~ pdq(2,0,2) + PDQ(1,1,1)),
